...
fn main() {
    ...
    // };
    let formula = parse_formula::parse_string_to_formula(&"=SUM({1,2,3},4,{5,6})");
    //let formula = parse_formula::parse_string_to_formula(&"=SUM(A,B,C)");
    println!("formula is {:?}", formula);
    let result = calculate::calculate_formula(formula, None::<NoFormula>); //Some(&data_function));
    println!("result is {:?}", result);
    println!("final result is {}", calculate::result_to_string(result));

    //////////////////// code that show on github /////////////////////////
